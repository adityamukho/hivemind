...
compoundEvents.truncate()
  ...
  .then(result => {
    for (const meta of result) {
      ...
      // Creates and Updates
      rg.post('/event/log', { path, postFilter: '(["created", "updated"]).includes(event)' },
        { groupBy: 'event', groupSort: 'asc' }).then(response => {
        ...
        for (const eventGroup of eventGroups) {
          ...
          if (cEvents.length) {
            compoundEvents.saveAll(cEvents).then(result => {
              console.log(
      ...
      // Deletes
      rg.post('/event/log', { path, postFilter: 'events[0].event === "deleted"' },
        { groupBy: 'node', groupLimit: 1 }).then(response => {
        const eventGroups = response.body
        ...
        function delSubTree () {
          ...
          `
          db.query(query).then(cursor => cursor.next()).then(nodeMetas => {
            ...
            compoundEvents.save(cEvent).then(() => console.log(
